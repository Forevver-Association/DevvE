...
describe("DevvE", function () {
  ...
  describe("Minting", function () {
    it("Should allow the minter to mint within limit", async function () {
      await devvE.connect(accounts[1]).mint(accounts[2].address, ethers.utils.parseEther("1000"));
      expect(await devvE.balanceOf(accounts[2].address)).to.equal(ethers.utils.parseEther("1000"));
      expect(await devvE.getMintedAmountBy(accounts[1].address)).to.equal(ethers.utils.parseEther("1000"));
    });

    it("Should not allow non-minter to mint", async function () {
      await expect(
        devvE.connect(accounts[3]).mint(accounts[3].address, ethers.utils.parseEther("1000"))
      ).to.be.revertedWith("Caller does not have the minter role");
    });

    it("Should not allow minting above global limit", async function () {
      const amount = ethers.utils.parseEther("210000000001");
      await expect(
    ...
    it("Should not allow minter to mint above individual limit", async function () {
      const amount = ethers.utils.parseEther("150000001")*(10**18);
      await expect(
    ...
    it("Should update the amount minted by minter correctly", async function () {
      ...
      await devvE.connect(minter).mint(receiver.address, ethers.utils.parseEther("2000"));
      expect(await devvE.getMintedAmountBy(minter.address)).to.equal(ethers.utils.parseEther("2000"));
    });
  });
  ...
  describe("Minter limits", function () {
    ...
    it("Should enforce minter limits correctly with multiple minters", async function () {
      ...
      await devvE.connect(minter1).mint(receiver.address, ethers.utils.parseEther("1000"));
      await devvE.connect(minter2).mint(receiver.address, ethers.utils.parseEther("2000"));
      expect(await devvE.balanceOf(receiver.address)).to.equal(ethers.utils.parseEther("3000"));
    });
  });
});
...
describe("DevvE Contract Upgrade", function () {
  ...
  it("Should have burn function in V2", async function () {
    ...
    await devvE.connect(accounts[1]).mint(accounts[0].address, ethers.utils.parseEther("1000"));
    await devvE_V2.connect(accounts[0]).burn(ethers.utils.parseEther("1000"));
